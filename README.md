...
- `全開／全停／全關` 共用「開啟／捲軸色」作為一致的操作色；pending 與 disabled 仍以按鈕狀態呈現。

### 背景氛圍

四種既有的 `theme` YAML 值保持向後相容，但已重新設計為同一套低彩度、霧面農業控制台風格：

| YAML 值 | 視覺化設定名稱 | 氛圍 |
|---|---|---|
| `dark` | 深夜石墨 | 中性深灰、低眩光 |
| `light` | 雲霧白 | 柔和亮面、清楚深色文字 |
| `greenhouse` | 森林深綠 | 克制的溫室綠、避免大片高飽和色 |
| `sand` | 暖陶米 | 低彩度暖米色、降低黃色背景干擾 |

四種氛圍共用相同的層次、表面透明度與局部光暈語言；切換主題只改變環境色調，不改變按鈕的操作語意。
